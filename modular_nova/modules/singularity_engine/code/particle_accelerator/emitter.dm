	...
	if(COOLDOWN_FINISHED(src, fire_delay_cooldown))
		...
		switch(strength)
			if(PARTICLE_STRENGTH_WEAK)
				...
				playsound(src, 'modular_nova/modules/singularity_engine/sound/cyclotron.ogg', 30, FALSE)
			if(PARTICLE_STRENGTH_NORMAL)
				new_particle = new/obj/effect/accelerated_particle(our_turf)
				playsound(src, 'modular_nova/modules/singularity_engine/sound/cyclotron.ogg', 70, FALSE)
			if(PARTICLE_STRENGTH_STRONG)
				new_particle = new/obj/effect/accelerated_particle/strong(our_turf)
				playsound(src, 'modular_nova/modules/singularity_engine/sound/cyclotron.ogg', 100, FALSE)
