...
/client/proc/cmd_mentor_pm(whom, msg)
	...
	if(is_mentor(whom))
		to_mentors(span_purple(span_mentor("[src] has started replying to [whom]'s mhelp.")))
	...
	if(!msg)
		...
		if(!msg)
			if (is_mentor(whom))
				to_mentors(span_mentor(span_purple("[src] has stopped their reply to [whom]'s mhelp.")))
			return
	...
	if(!msg)
		if (is_mentor(whom))
			to_mentors(span_mentor(span_purple("[src] has stopped their reply to [whom]'s mhelp.")))
		return
	log_admin_private("Mentor PM: [key_name(src)]->[key_name(target)]: [msg]")
	...
	for(var/it in get_admin_counts(R_MENTOR)["total"])
